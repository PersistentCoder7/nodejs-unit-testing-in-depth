...
describe('mailer', () => {
    ...
    context('sendWelcomeEmail', () => {
        ...
        it('should check for password reset email and name', async() => {
            await expect(mailer.sendPasswordResetEmail()).to.eventually.be.rejectedWith('Invalid input');
            //await expect(mailer.sendPasswordResetEmail('foo@bar.com')).to.eventually.be.rejectedWith('Invalid input');
        });

        it('should call sendEmail with password and message', async() => {
